...
function init () {
  ...
  // updateTitle(title)
  title = 'List of National Natural Landmarks in Colorado - Wikipedia'
  updateTitleUI()
  if (isShown) show()
  else hide()
  console.log('[handle] ✔')
...
function updateTitleUI () {
  ...
  titleEl.classList.remove('align-buttons')
  titleWrapper.classList.remove('align-buttons')

  if (titleX < buttonsWidth + 5) { // 5 margin to avoid jitter
    // even centered with buttons, 
    // title would overlay them
